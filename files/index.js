...
function parseMediaType(str, i) {
	...
	if (match[3]) {
		...
		for (var j = 0; j < kvps.length; j++) {
			...
			var value =
				val && val[0] === '"' && val[val.length - 1] === '"' ? val.substr(1, val.length - 2) : val;
...
function chunkLength(chunk, encoding) {
	...
	return !Buffer$1.isBuffer(chunk) ? Buffer$1.byteLength(chunk, encoding) : chunk.length;
}
...
function toBuffer(chunk, encoding) {
	return !Buffer$1.isBuffer(chunk) ? Buffer$1.from(chunk, encoding) : chunk;
}
...
const server = polka().use(compression$1()).use(handler);
